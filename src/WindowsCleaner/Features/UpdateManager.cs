...
namespace WindowsCleaner
{
    ...
    public class UpdateManager
    {
        ...
        public async Task<UpdateInfo?> CheckForUpdateAsync()
        {
            try
            {
                Logger.Log(LogLevel.Info, $"🔍 Vérification des mises à jour...");
                ...
                {
                    Logger.Log(LogLevel.Warning, $"⚠️ Impossible de vérifier les mises à jour (HTTP {response.StatusCode})");
                    return null;
                }
                ...
                {
                    Logger.Log(LogLevel.Warning, "⚠️ Aucune version disponible sur GitHub");
                    return null;
                }
                ...
                {
                    Logger.Log(LogLevel.Info, $"✨ Nouvelle version disponible : {latestVersion} (actuelle: {_currentVersion})");
                ...
                {
                    Logger.Log(LogLevel.Info, $"✅ Vous utilisez la dernière version ({_currentVersion})");
                    return null;
                }
            }
            catch (HttpRequestException ex)
            {
                Logger.Log(LogLevel.Error, $"❌ Erreur réseau lors de la vérification : {ex.Message}");
                return null;
            }
            catch (Exception ex)
            {
                Logger.Log(LogLevel.Error, $"❌ Erreur lors de la vérification : {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<bool> DownloadUpdateAsync(UpdateInfo updateInfo, string destinationPath, IProgress<int>? progress = null)
        {
            try
            {
                ...
                {
                    Logger.Log(LogLevel.Error, "❌ Aucun fichier de téléchargement disponible");
                    return false;
                }

                Logger.Log(LogLevel.Info, $"📥 Téléchargement de la mise à jour depuis : {updateInfo.DownloadUrl}");
                ...
                Logger.Log(LogLevel.Info, $"✅ Téléchargement terminé : {destinationPath}");
                return true;
            }
            catch (Exception ex)
            {
                Logger.Log(LogLevel.Error, $"❌ Erreur lors du téléchargement : {ex.Message}");
                return false;
            }
        }
        ...
        public void OpenReleaseUrl(string url)
        {
            try
            {
                ...
                Logger.Log(LogLevel.Info, $"🌐 Page de release ouverte : {url}");
            }
            catch (Exception ex)
            {
                Logger.Log(LogLevel.Error, $"❌ Impossible d'ouvrir le navigateur : {ex.Message}");
            }
        }
